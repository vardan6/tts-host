    ...
    while (true) {
      ...
      for (const auto &message :
        ...
        if (method == "synthesize") {
          ...
          const auto frames = runner.run_synthesis(request.text, request.speed);
          ...
          response = runner.make_synthesize_response(message, frames);
          // Send the control response before audio frames: the host blocks on
          // the control response before it starts draining the audio pipe, so
          // writing a (possibly large) audio frame first
          ...
          const auto audio_output = tts_host::RunnerAudioOutput::open_inherited();
          for (const auto &frame : frames) {
            audio_output.write_frame(frame);
          }
          continue;
        } else if (method == "load") {
